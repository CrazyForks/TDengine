...
static int32_t periodTriggerToJson(const void* pObj, SJson* pJson) {
  ...
  TAOS_CHECK_RETURN(tjsonAddStringToObject(
    pJson, jkPeriodTriggerPeriodUnit, (const char*)&pTrigger->periodUnit));
  TAOS_CHECK_RETURN(tjsonAddStringToObject(
    pJson, jkPeriodTriggerOffsetUnit, (const char*)&pTrigger->offsetUnit));
  TAOS_CHECK_RETURN(tjsonAddIntegerToObject(
...
int32_t scmCreateStreamReqToJsonImpl(const void* pObj, void* pJson) {
  ...
  if (NULL != pReq->subTblNameExpr) {
    TAOS_CHECK_RETURN(tjsonAddStringToObject(
      pJson, jkCreateStreamReqSubTblNameExpr, (const char*)pReq->subTblNameExpr));
  }
...
int32_t scmCreateStreamReqToJson(
  const SCMCreateStreamReq* pReq, bool format, 
  char** ppStr, int32_t* pStrLen) {
  int32_t code = TSDB_CODE_SUCCESS;
  int32_t lino = 0;
  int64_t streamId = pReq ? pReq->streamId : -1;
  STREAM_CHECK_NULL_GOTO(pReq, TSDB_CODE_MND_STREAM_INTERNAL_ERROR);
  STREAM_CHECK_NULL_GOTO(ppStr, TSDB_CODE_MND_STREAM_INTERNAL_ERROR);
